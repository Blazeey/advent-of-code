...
fn generate(grid: &[Vec<char>], start_row: i32, start_col: i32, dir: Dir) -> usize {
    let m = grid.len();
    let n = grid[0].len();
    ...
    new_grid.iter().flatten().filter(|f| **f).count()
}
...
fn main() {
    ...
    let n = grid[0].len();
    let top_max = (0..n)
        .map(|i| generate(&grid, 0, i as i32, Dir::U))
        .max()
        .unwrap();
    let bottom_max = (0..n)
        .map(|i| generate(&grid, m as i32 - 1, i as i32, Dir::D))
        .max()
        .unwrap();
    let left_max = (0..m)
        .map(|i| generate(&grid, i as i32, 0, Dir::L))
        .max()
        .unwrap();
    let right_max = (0..m)
        .map(|i| generate(&grid, i as i32, n as i32 - 1, Dir::R))
        .max()
        .unwrap();

    let maxxes = [top_max, bottom_max, left_max, right_max];
    let ans = maxxes.iter().max().unwrap();
